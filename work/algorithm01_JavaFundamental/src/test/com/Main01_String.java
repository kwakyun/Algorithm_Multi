...
public class Main01_String {
    public static void main(String[] args) throws IOException {
        ...
        //미션- 현재 여려분들이 알고있는 문자열과 관련된 객체와 함수를 모두 적어보세요.
...
//        System.out.println(new String(cs));  //cs -> String >> java:jsp:spring
        ...
        System.out.println(Arrays.toString(cs)); // Arrays 로 출력가능.

        //6. 문자열 뽑기(자르기, 추출)
        System.out.println(temp.substring(5)); //5번 인덱스부터 끝까지 >> jsp:spring
        System.out.println(temp.substring(temp.lastIndexOf("j"))); //마지막j부터 끝까지 >> jsp:spring
        System.out.println(temp.substring(temp.lastIndexOf(":"))); //마지막콜론부터 인덱스부터 끝까지 >> :spring
        System.out.println(temp.substring(temp.lastIndexOf(":") + 1)); //마지막콜론 다음부터 인덱스부터 끝까지 >> spring
        System.out.println(temp.substring(5, 7 + 1)); //5부터 원하는 인덱스+1  : jsp
        System.out.println(temp.substring(temp.indexOf(":") + 1, temp.lastIndexOf(":"))); //:첫번째 인덱스부터 : 마지막 인덱스 전 까지 :    jsp
        System.out.println();

        temp = "java11:jsp22:spring33:javascript44:CSS55:HTML66:한글";
        System.out.println(temp);
        //indexOf와 lastIndexOf만으로는 한계가 있다.
        System.out.println(temp.substring(temp.indexOf(":") + 1, temp.lastIndexOf(":")));

        //7. 구분자(패턴)를 이용해서 문자열을 분리해서 배열로 반환
        String[] arr = temp.split(":");
        System.out.println(Arrays.toString(arr));
        System.out.println(arr[1]); //jsp

        //8. 문자열 변경 후 반환 (원본이 바뀌지 않는다.)
        ...
        System.out.println(temp.equalsIgnoreCase("Yangssem")); //대소문자 구분 안함
        ...
        System.out.println(temp.compareToIgnoreCase("Yangssem")); //대소문자 구분안함 같으면
        ...
        //13. 문자열 대용량으로 병합(연산횟수가 많은 문자열)- 엄청난 연산속도
//        StringBuffer sb = new StringBuilder();
        StringBuilder sb = new StringBuilder();
        sb.append(str1);
        sb.append(str2);
        System.out.println(sb.toString());
        //단순병합 속도와 대용량병합 속도 비교
...
//        temp = "   java   ";
        temp = "   ja va   "; // 가운대 있는 공백은 제거 X , 모두 제거하고 싶으면 replace 사용
        System.out.println("temp.length() = " + temp.length());
        ...
        for (int i = 0; i <= (temp.length() - 1) / 2; i++) {
            char cTemp;
            cTemp = tempArr[i];
            tempArr[i] = tempArr[temp.length() - 1 - i];
            tempArr[temp.length() - 1 - i] = cTemp;
        ...
        System.out.println(answer);
        System.out.println(new StringBuilder(temp).reverse().toString()); //가장 편리하다.
        System.out.println(answer);
        ...
        System.out.println(temp.isEmpty()); //"", 빈문자인지 검증
        temp = "     ";
        ...
        //22. 시작하는 문자열이 영문 대소문자 또는 숫자 형태인지 패턴으로 검증.
        temp = "7Aabc";
